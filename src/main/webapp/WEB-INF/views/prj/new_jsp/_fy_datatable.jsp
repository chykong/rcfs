                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        if (row.parent_type == 0) {
                            ...
                            return '<button data-map="' + row.map_id + '" class="btn-sm btn btn-primary btn-white  open-subHome">\
                            <a href="<c:url value="/prj/preallocation/basic/view.htm"/>?id=' + row.id + '">' + data + '</a>\
...
//                $("#foo").addClass('hidden');
        ...
        $('#tbody').on('click', '.open-subHome', function () {
            var $tr = $(this).closest('tr');
            var map_id = $(this).attr("data-map");
            var $append_item =$('#tbody').find('.append-item');
            $append_item.remove();

            var $i = $(this).find('i');
            if($i.hasClass('fa-angle-down')){
                $.get('<c:url value="/prj/preallocation/basic/getSubHome"/>?map_id=' + map_id,function(result){
                    $tr.after(result);
                });
                var $open_i =$('#tbody').find('.fa-angle-up');
                $open_i.removeClass('fa-angle-up').addClass('fa-angle-down');
