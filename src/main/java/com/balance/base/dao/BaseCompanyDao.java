...
public class BaseCompanyDao extends BaseDao<BaseCompany, BaseCompanySearchVO> {
	...
	public int add(BaseCompany baseCompany) {
		String sql = "insert into t_base_company(name,type,created_at,created_by,prj_base_info_id) values(:name,:type,now(),:created_by,:prj_base_info_id)";
	...
	public List<BaseCompany> list(BaseCompanySearchVO baseCompanySearchVO) {
		...
		sql+= createSarchSql(baseCompanySearchVO);
	...
	public String createSarchSql(BaseCompanySearchVO baseCompanySearchVO) {
		String sql = "";
		if (baseCompanySearchVO.getPrj_base_info_id() != null) {
			sql += " where  prj_base_info_id=:prj_base_info_id";// 项目id
		}
		return sql;
	}
}
