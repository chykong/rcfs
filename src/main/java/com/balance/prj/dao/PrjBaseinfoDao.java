...
public class PrjBaseinfoDao extends BaseDao<PrjBaseinfo, PrjBaseinfoSearchVO> {
    ...
    public int add(PrjBaseinfo prjBaseinfo) {
        String sql = "insert into t_prj_baseinfo(prj_name,introduction,progress,created_at,created_by,area_type,stat_type) values(:prj_name,:introduction,:progress,now(),:created_by,:area_type,:stat_type)";
    ...
    public int update(PrjBaseinfo prjBaseinfo) {
        String sql = "update t_prj_baseinfo set prj_name=:prj_name,progress=:progress,last_modified_at=now(),last_modified_by=:last_modified_by,area_type=:area_type,stat_type=:stat_type where id=:id";
        return update(sql, prjBaseinfo);
    }
