...
public class PrjReportDao extends BaseDao<PrjReport, PrjReportSearchVO> {
    ...
    private String createSearchSql(PrjReportSearchVO prjReportSearchVO) {
        String sql = "";
        if (prjReportSearchVO.getProgress() != null) {//名称模糊查询
    ...
    public int update(PrjReport prjReport){
    	String sql="update t_prj_report set progress=:progress,title=:title,content=:content,last_modified_at=now(),last_modified_by=:last_modified_by where id=:id";
    	return update(sql,prjReport);
    }
    /**
     * 删除会议纪要
